...
contract kyc {
    ...
    function addCustomer(string Uname, string DataHash) public payable returns(uint) {
        ...
        if(allCustomers.length < 1)
            return 1;
        allCustomers[allCustomers.length-1] = Customer(Uname, DataHash, 100, 0, msg.sender, "null", "null");
    ...
    function setPhoneNumber(string Uname, string password, string phone ) public payable returns(bool) {
        for(uint i=0;i < allCustomers.length; ++ i) {
            if(stringsEqual(allCustomers[i].uname, Uname) && stringsEqual(allCustomers[i].password, "null") && stringsEqual(allCustomers[i].number, "null")) {
                allCustomers[i].number = phone;
                return true;
            }
        }
        return false;
